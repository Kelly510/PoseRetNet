...
class ChunkedGenerator_Seq:
    ...
    def __init__(self, batch_size, cameras, poses_3d, poses_2d,
                 chunk_length, stride, shuffle=True, random_seed=1234, random_shift=False,
        ...
        for i in range(len(poses_2d)):
            ...
            n_chunks = max((poses_2d[i].shape[0] - chunk_length) // stride, 1)
            starts = (np.arange(n_chunks) * stride).tolist()
            ends = (np.arange(n_chunks) * stride + chunk_length).tolist()
            if n_chunks * stride < poses_2d[i].shape[0] - chunk_length:
                starts.append(poses_2d[i].shape[0] - chunk_length)
                ends.append(poses_2d[i].shape[0])
                n_chunks += 1

            augment_vector = np.full(n_chunks, False, dtype=bool)
            pairs += zip(np.repeat(i, n_chunks), starts, ends, augment_vector)
            if augment:
                pairs += zip(np.repeat(i, n_chunks), starts, ends, ~augment_vector)
            ...
            for b_i in range(start_idx, self.num_batches):
                ...
                for i, (seq_i, start_3d, end_3d, flip) in enumerate(chunks):
                    ...
                    if self.random_shift:
                        # 需要满足经过random_shift之后，chunk的开始帧不超出整个序列，chunk的结束帧不比0小
                        upper_bound = min(self.stride // 2, seq_2d.shape[0] - start_3d - 1)
                        lower_bound = max(-self.stride // 2, - end_3d + 1)
                        random_shift = np.random.randint(lower_bound, upper_bound)
                        start_3d += random_shift
                        end_3d += random_shift
                        assert start_3d < seq_2d.shape[0]
                        assert end_3d > 0
