...
class STCModel(nn.Module):
    def __init__(self, num_frame=9, num_joints=17, in_chans=2, embed_dim_ratio=512, depth=4,
        ...
        layers, d_hid, frames = depth, embed_dim_ratio, num_frame
...
if __name__ == "__main__":
    ...
    net = STCModel(num_frame=27, num_joints=17, in_chans=2, embed_dim_ratio=256, depth=6,
                   num_heads=8, mlp_ratio=2., qkv_bias=True, qk_scale=None,
                   drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,  norm_layer=None, 
                   gamma_divider=8, joint_related=False, trainable=False, chunk_size=None, 
                   causal=True, dataset='h36m').cuda()
    inputs = torch.rand([1, 27, 17, 2]).cuda()
    output = net(inputs)
    print(output.size())
